...
{$IFNDEF VER120}
      ...
      function  LerRetorno_bb(var ACobranca: TgbCobranca; Retorno: TStringList) : boolean; {Lê o arquivo retorno recebido do banco}
...
begin
   ...
   begin
     ...
     // else
     //    Registro := Registro + Formatar('REMESSA-PRODUCAO',20); {192 a 211 - Deverá conter a literal REMESSA-TESTE para fase de testes}
...
begin
   ...
   TRY
      ...
      begin
         ...
         NumeroRegistro := NumeroRegistro + 1;
         {Lê até o antepenúltimo registro porque o penúltimo contém apenas o TRAILER DO LOTE e o último contém apenas o TRAILER DO ARQUIVO E 2 EM BRANCO }
...
begin
  ...
  // Acobranca.DataArquivo := Sistema.DataAtual;
   ...
   begin
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.CodigoAgencia,5,false,'0'); {53 a 57 - Código da agência do cedente}
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.DigitoAgencia,1,false,'0'); {58 - Dígito do código da agência do cedente}
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,6,false,'0'); {59 a 64 - Código dom cedente no banco}
      ...
      Registro := Registro + Formatar(IntToStr(NumeroArquivo),6,false,'0'); {158 a 163 Número seqüencial do arquivo}
      Registro := Registro + '050'; {164 a 166 - Número da versão do layout do arquivo}
      Registro := Registro + Formatar(' ',5,false,'0'); {167 a 171 - Densidade de gravação do arquivo (BPI)}
      Registro := Registro + Formatar(' ',20); {172 a 191 - Uso reservado do banco}
      ...
      if TipoMovimento = tmRemessaTeste then
        ...
        Registro := Registro + Formatar('REMESSA-PRODUCAO',20); //Formatar('',20); {192 a 211 - Deverá conter a literal REMESSA-TESTE para fase de testes}
     ...
     {GERAR REGISTRO HEADER DO LOTE}
      ...
      Registro := Registro + '1'; {8 - Tipo do registro - Registro header de lote}
      Registro := Registro + 'R'; {9 - Tipo de operação: R (Remessa) ou T (Retorno)}
      Registro := Registro + '02'; {10 a 11 - Tipo de serviço: 01 (Cobrança)}
      Registro := Registro + '00'; {12 a 13 - Forma de lançamento: preencher com ZEROS no caso de cobrança}
      ...
      {CÓDIGO DO CONVÊNIO = AGÊNCIA + NÚMERO CONVÊNIO + DV}
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,6,false,'0'); {34 a 39 - Código do convênio no banco}
      Registro := Registro + '00000000000000'; {40 a 53 - Uso exclusivo CAIXA}
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.CodigoAgencia,5,false,'0'); {54 a 58 - Código da agência do cedente}
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.DigitoAgencia,1,false,'0'); {59 - Dígito da agência do cedente}
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,6,false,'0'); {60 a 65 - Código do Convênio no Banco}
      Registro := Registro + '0000000'; {66 a 72 - Código do Modelo Personalizado}
      ...
      begin
         ...
         Registro := Registro + Formatar(IntToStr(NumeroLote),4,false,'0'); {4 a 7 - Número do lote}
         Registro := Registro + '3'; {8 - Tipo do registro: Registro detalhe}
         Registro := Registro + Formatar(IntToStr(X),5,false,'0'); {9 a 13 - Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)}
         Registro := Registro + 'P'; {14 - Código do segmento do registro detalhe}
         Registro := Registro + ' '; {15 - Uso exclusivo FEBRABAN/CNAB: Branco}
         case Titulos[NumeroRegistro].TipoOcorrencia of {16 a 17 - Código de movimento}
            toRemessaRegistrar                 : Registro := Registro + '01';
         ...
         Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.CodigoAgencia,5,false,'0'); {18 a 22 - Agência mantenedora da conta}
         Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.DigitoAgencia,1,false,'0'); {23 - Dígito verificador da agência}
         Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,6,false,'0'); {24 a 29 - Código do Convênio no Banco}
         Registro := Registro + '00000000000'; {30 a 40 - Uso Exclusivo da CAIXA}
//         if Sistema.cdsEmpresaEMP_NOME_ABREV.value='CORE-RN' THEN
            Registro := Registro + copy(Titulos[NumeroRegistro].NossoNumero,1,2) + copy(Titulos[NumeroRegistro].NossoNumero,4,15); {41 a 57 - modalidade da carteira (24) + Nosso número - identificação do título no banco}
//         ELSE
//            Registro := Registro + Titulos[NumeroRegistro].NossoNumero; {41 a 57 - modalidade da carteira (24) + Nosso número - identificação do título no banco}
         Registro := Registro + '1'; {58 - Cobrança Simples}
         Registro := Registro + '2'; {59 - Forma de cadastramento do título no banco: com cadastramento}
         Registro := Registro + '2'; {60 - Tipo de documento: Escritural}
         case Titulos[NumeroRegistro].EmissaoBoleto of {61 a 62 - Quem emite e quem distribui o boleto?}
            ebBancoEmite      : Registro := Registro + '1' + '1';
            ebClienteEmite    : Registro := Registro + '2' + '2';
         ...
         //Registro := Registro + copy(Titulos[NumeroRegistro].SeuNumero,1,7) + copy(Titulos[NumeroRegistro].SeuNumero,9,4); {63 a 73 - Número que identifica o título na empresa}
         ...
         if Titulos[NumeroRegistro].ValorMoraJuros > 0 then
         begin
            Registro := Registro + '3'; {118 - Código de juros de mora: Valor por dia}
            if Titulos[NumeroRegistro].DataMoraJuros <> Null then
               Registro := Registro + FormatDateTime('ddmmyyyy',Titulos[NumeroRegistro].DataMoraJuros) {119 a 126 - Data a partir da qual serão cobrados juros}
            else
               Registro := Registro + Formatar(' ',8,false,'0'); {119 a 126 - Data a partir da qual serão cobrados juros}
         ...
         begin
            Registro := Registro + '3'; {118 - Código de juros de mora: Acata cadastramento na CAIXA}
            Registro := Registro + Formatar(' ',8,false,'0'); {119 a 126 - Data a partir da qual serão cobrados juros}
            Registro := Registro + Formatar(' ',15,false,'0'); {127 a 141 - Valor de juros de mora por dia}
         end;
         ...
         Registro := Registro + Formatar(STRINGREPLACE(Titulos[NumeroRegistro].SeuNumero,'/','',[RFREPLACEALL]),25); {196 a 220 - Identificação do título na empresa}
         ...
         Registro := Registro + Formatar(IntToStr(NumeroLote),4,false,'0'); {Número do lote}
         Registro := Registro + '3'; {Tipo do registro: Registro detalhe}
         Registro := Registro + Formatar(IntToStr(X),5,false,'0'); {Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)}
         Registro := Registro + 'Q'; {Código do segmento do registro detalhe}
         Registro := Registro + ' '; {Uso exclusivo FEBRABAN/CNAB: Branco}
         case Titulos[NumeroRegistro].TipoOcorrencia of {Código de movimento}
            toRemessaRegistrar                 : Registro := Registro + '01';
        ...
        // Dados do sacador/avalista}
         ...
         Registro := Registro + '0'; {Identificação Manutenção
         ...
         {SEGMENTO R -  para impressão da mensagem - instrução}
         Registro := Formatar(CodigoBanco,3,false,'0'); {Código do banco}
         Registro := Registro + Formatar(IntToStr(NumeroLote),4,false,'0'); {Número do lote}
         Registro := Registro + '3'; {Tipo do registro: Registro detalhe}
         Registro := Registro + Formatar(IntToStr(x),5,false,'0'); {Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)}
         ...
         {SEGMENTO S -  para impressão da mensagem - instrução}
         Registro := Formatar(CodigoBanco,3,false,'0'); {Código do banco}
         Registro := Registro + Formatar(IntToStr(NumeroLote),4,false,'0'); {Número do lote}
         Registro := Registro + '3'; {Tipo do registro: Registro detalhe}
         Registro := Registro + Formatar(IntToStr(x),5,false,'0'); {Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)}
...
begin
    ...
    begin
        ...
        Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.CodigoAgencia, 5, false, '0'); {53 a 57 - Código da agência do cedente}
        Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.DigitoAgencia, 1, false, '0'); {58 - Dígito do código da agência do cedente}
        Registro := Registro + '0'+Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente, 5, false, '0'); {59 a 64 - Código dom cedente no banco}
        Registro := Registro + '00000000'; // 65 a 72 - Uso Exclusivo CAIXA
        ...
        Registro := Registro + Formatar(IntToStr(NumeroArquivo), 6, false, '0'); {158 a 163 Número seqüencial do arquivo}
        Registro := Registro + '101'; {164 a 166 - Número da versão do layout do arquivo}
        Registro := Registro + Formatar(' ', 5, false, '0'); {167 a 171 - Densidade de gravação do arquivo (BPI)}
        Registro := Registro + Formatar(' ', 20); {172 a 191 - Uso reservado do banco}
        ...
        Registro := Registro + '01'; {10 a 11 - Tipo de serviço: 01 (Cobrança)}
        Registro := Registro + '00'; {12 a 13 - Forma de lançamento: preencher com ZEROS no caso de cobrança}
        Registro := Registro + '060'; {14 a 16 - Número da versão do layout do lote}
        Registro := Registro + ' '; {17 - Uso exclusivo FEBRABAN/CNAB}
        Registro := Registro + ACedenteTipoInscricao; {18 - Tipo de inscrição do cedente}
        Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.NumeroCPFCGC, 14, false, '0'); {19 a 32 - Número de inscrição do cedente}

        {CÓDIGO DO CONVÊNIO = AGÊNCIA + NÚMERO CONVÊNIO + DV}
        Registro := Registro + '0'+Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente, 5, false, '0'); {33 a 38 - Código do convênio no banco}
      ...
      //  Registro := Registro + FormatDateTime('ddmmyyyy', DataArquivo); {200 a 207 - Data do crédito - Informar a mesma data da gravação do arquivo}
        ...
        begin
            ...
            Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.CodigoAgencia, 5, false, '0'); {18 a 22 - Agência mantenedora da conta}
            Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.DigitoAgencia, 1, false, '0'); {23 - Dígito verificador da agência}
            Registro := Registro + '0'+Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente, 5, false, '0'); {24 a 29 - Código do Convênio no Banco}
            Registro := Registro + '00000000000'; {30 a 40 - Uso Exclusivo da CAIXA}
            Registro := Registro + Titulos[NumeroRegistro].NossoNumero; {41 a 57 - modalidade da carteira (24) + Nosso número - identificação do título no banco}
            Registro := Registro + '1'; {58 - Cobrança Simples}
            Registro := Registro + '1'; {59 - Forma de cadastramento do título no banco: com cadastramento}
            ...
            Registro := Registro + '2'; {118 - Código de juros de mora: Acata cadastramento na CAIXA}
            Registro := Registro + FormatDateTime('ddmmyyyy', IncDay(Titulos[NumeroRegistro].DataVencimento,1)); {119 a 126 - Data a partir da qual serão cobrados juros}
            ...
            Registro := Registro + '1'; {224 - Código para baixa/devolução: Baixar/devolver}
            Registro := Registro + '005'; {225 a 227 - Prazo para baixa/devolução (em dias corridos)}
...
begin
    ...
    begin
        ...
        Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.CodigoAgencia, 5, false, '0'); {53 a 57 - Código da agência do cedente}
        Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.DigitoAgencia, 1, false, '0'); {58 - Dígito do código da agência do cedente}
        Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente, 7, false, '0'); {59 a 65 - Código dom cedente no banco}
        ...
        Registro := Registro + '1'; {144 - Código de Remessa (1) / Retorno (2)}
        Registro := Registro + FormatDateTime('ddmmyyyy', DataArquivo); {145 a 152 - Data do de geração do arquivo}
        Registro := Registro + FormatDateTime('hhmmss', DataArquivo); {153 a 158 - Hora de geração do arquivo}

        Registro := Registro + Formatar(IntToStr(NumeroArquivo), 6, false, '0'); {159 a 164 Número seqüencial do arquivo}
        Registro := Registro + '101'; {165 a 167 - Número da versão do layout do arquivo}
        Registro := Registro + Formatar(' ', 5, false, '0'); {168 - 172 - Densidade de gravação do arquivo (BPI)}
      ...
      //  Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente, 5, false, '0'); {174 - 178 -  Código sindical da Entidade Sindical}
        ...
        Registro := Registro + Formatar(' ', 4); {199 - 202 Versão Aplicativo CAIXA}
        ...
        {CÓDIGO DO CONVÊNIO = AGÊNCIA + NÚMERO CONVÊNIO + DV}
        Registro := Registro +Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente, 7, false, '0'); {33 a 39 - Código do convênio no banco}
...
//        Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente, 5, true, '0'); {69 - 73 -  Código sindical da Entidade Sindical}
        ...
        Registro := Registro + Formatar(IntToStr(NumeroArquivo), 12, false, '0'); {186 a 197 - Número do arquivo}
        Registro := Registro + FormatDateTime('ddmmyyyy', DataArquivo); {198 a 205 - Data de geração do arquivo}
        Registro := Registro + FormatDateTime('ddmmyyyy', DataArquivo); {206 a 213 - Data do crédito - Informar a mesma data da gravação do arquivo}
        ...
        begin
            ...
            Registro := Registro + '3'; {118 - Código de juros de mora: Acata cadastramento na CAIXA - 3 = isento orientacao do manual}
            //Registro := Registro + FormatDateTime('ddmmyyyy', IncDay(Titulos[NumeroRegistro].DataVencimento,1)); {119 a 126 - Data a partir da qual serão cobrados juros}
            Registro := Registro + '00000000'; {119 a 126 - Data a partir da qual serão cobrados juros}
            ...
            Registro := Registro + '1'; {224 - Código para baixa/devolução: Baixar/devolver}
            Registro := Registro + '090'; {225 a 227 - Prazo para baixa/devolução (em dias corridos)}
