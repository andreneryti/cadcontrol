...
uses
   Windows, classes, SysUtils, Graphics, extctrls,Variants, Printers,
   MaskUtils, contnrs, NMSMTP, gbBoleto6, jvdateutil,gbBoleto_A4SIGCB_new,gbBoleto_A4SIGCB_mg,gbBoleto_A4SIGCB_BB_MG,QuickRpt;
...
type
   ...
   private
      ...
      fDigito : string; {Código do banco na câmara de compensação}
      procedure SetCodigo(ACodigoBanco : string);
      function GetDigito : string; {Retorna o dígito do código do banco}
      function GetNome   : string;
    procedure SetDigito(const Value: string); {Retorna o nome do banco}
   public
   ...
   procedure PrepararBoleto_sigcb_mg(ABoleto: TBoleto_A4SIGCB_MG);
   procedure PrepararBoleto_sigcb_BB_mg(ABoleto: TBoleto_A4SIGCB_BB_MG);
   procedure PrepararBoleto_sigcb_BB_mg_CTA2(ABoleto: TBoleto_A4SIGCB_BB_MG);
   private
      ...
      FBoleto_SIGCB_MG : TBoleto_A4SIGCB_MG;
      FBoleto_SIGCB_BB_MG : TBoleto_A4SIGCB_BB_mg;
      ...
      procedure PrepararBoleto_sigcb(ABoleto: TBoleto_A4SIGCB);
      ...
      procedure QRCompoBoletoAddReports(Sender: TObject);
   public
      ...
      procedure Visualiazar_SIGCB;
      PROCEDURE Visualiazar_SIGCB_e_PDF(nomearquivo:string);
      function GerarPDF_SIGCB(caminho, arquivo:string):integer;
      procedure Visualiazar_SIGCB_MG;
      procedure Visualiazar_SIGCB_BB_MG;
      procedure Imprimir;
      PROCEDURE Imprimir_SIGCB;
      PROCEDURE Imprimir_SIGCB_e_PDF(nomearquivo:string);
      PROCEDURE Imprimir_SIGCB_MG;
      PROCEDURE Imprimir_SIGCB_BB_MG;
   ...
   published
      ...
      Property CurrentBoleto : TBoleto Read FBoleto Write FBoleto;
   end;
...
{$IFNDEF VER120}
   ...
   TLayoutArquivo = (laCNAB240, laCNAB400, laOutro, laMeuLay);
   ...
   public
      ...
      function LerRetorno_BB : boolean; {Lê o arquivo retorno recebido do banco}
      function LerRetorno_BB_AL : boolean; {Lê o arquivo retorno recebido do banco}
...
//uses gbCob001, gbCob104, gbCob748, uSistema {,
uses gbCob104, UFuncoes, UDMAssociado, UDMConexao;
var
   FBoleto_SIGCB_MG : TBoleto_A4SIGCB_MG;
...
begin
  ...
  Begin
    fCodigo := '';
    Fdigito := '';
  end else if (ACodigoBanco <> fCodigo) then
  Begin
...
procedure TgbBanco.SetDigito(const Value: string);
begin
  FDigito := Value;
end;
...
begin
  if Cedente.ContaBancaria.Banco.Codigo = '' then Cedente.ContaBancaria.Banco.Codigo := '104';
...
begin
   ...
   begin
      ...
      if copy(AAgenciaCodigoCedente,length(AAgenciaCodigoCedente),1)='-' then
         AAgenciaCodigoCedente:= copy(AAgenciaCodigoCedente,1,length(AAgenciaCodigoCedente)-1);
...
//      txtNomeBanco.Caption := Cedente.ContaBancaria.Banco.Nome;
      ...
      //ANossoNumero := inttostr(ExtractYear(now)) +copy(ANossoNumero,5,length(ANossoNumero)-4);
...
begin
   FBoleto := TBoleto.Create(nil);
   TRY
      PrepararBoleto(FBoleto);
      FBoleto.Preview;
      FBoleto.Free;
   EXCEPT
      FBoleto.Free;
...
begin
   ...
   TRY
      ...
      ABoleto.ReportTitle := NossoNumero;
      ABoleto.Print;
      ABoleto.Free;
   EXCEPT
      ABoleto.Free;
...
procedure TgbTitulo.Imprimir_SIGCB;
var
   ABoleto : TBoleto_A4SIGCB;
begin
   ABoleto := TBoleto_A4SIGCB.Create(nil);
   TRY
      PrepararBoleto_sigcb(ABoleto);
      ABoleto.ShowProgress := false;
      ABoleto.PrinterSettings.PrinterIndex := pRINTER.PrinterIndex;
      ABoleto.ReportTitle := NossoNumero;
      ABoleto.Print;
...
procedure TgbTitulo.PrepararBoleto_sigcb_mg(ABoleto: TBoleto_A4SIGCB_MG);
var
   AAgenciaCodigoCedente,
   ANossoNumero,
   ACarteira,
...
begin
   AInstrucoes := TStringList.Create;
   GetFormatoBoleto := nil;
   if cedente.DigitoCodigoCedente <> '' then
      AAgenciaCodigoCedente := Cedente.ContaBancaria.CodigoAgencia + '/' + FormatarComMascara('000000',Cedente.CodigoCedente) + '-'+ cedente.DigitoCodigoCedente
   else
      AAgenciaCodigoCedente := Cedente.ContaBancaria.CodigoAgencia + '/' + FormatarComMascara('000000',Cedente.CodigoCedente);
   ANossoNumero := NossoNumero + '-' + DigitoNossoNumero;
   ACarteira := Carteira;
   ...
   begin
      ...
      txtInstrucoesc.Lines.Assign(AInstrucoes);
      txtCodigoBanco_SIGCB.Caption := Cedente.ContaBancaria.Banco.Codigo + '-' + Cedente.ContaBancaria.Banco.Digito;
      txtLocalPagamento_SIGCB.Caption := AnsiUpperCase(LocalPagamento);

      txtDataVencimento_SIGCB.Caption := FormatDateTime('dd/mm/yyyy',DataVencimento);
      txtVencimento.Caption := FormatDateTime('dd/mm/yyyy',DataVencimento);

      txtNomeCedente_SIGCB.Caption := AnsiUpperCase(Cedente.Nome);
      txtNomeCedentec.caption :=AnsiUpperCase(Cedente.Nome);

      txtAgenciaCodigoCedentec.caption :=AAgenciaCodigoCedente;
      txtAgenciaCodigoCedente_SIGCB.Caption := AAgenciaCodigoCedente;

      txtNossoNumeroc.caption :=  ANossoNumero;
      txtNossoNum_SIGCB.caption :=  ANossoNumero;


      txtDataDocumento_SIGCB.Caption := FormatDateTime('dd/mm/yyyy',DataDocumento);
      txtDataDocumentoc.Caption := FormatDateTime('dd/mm/yyyy',DataDocumento);

      txtNumeroDocumento_SIGCB.Caption := NumeroDocumento;
      txtNumeroDocumentoc.Caption := NumeroDocumento;

      txtEspecieDocumento_SIGCB.Caption := AEspecieDocumento;
      txtEspecieDocumentoc.Caption := AEspecieDocumento;

      if AceiteDocumento = adSim then
      begin
         txtAceite_SIGCB.Caption := 'S';
         txtAceitec.Caption := 'S';
      end
      else
      begin
         txtAceite_SIGCB.Caption := 'N';
         txtAceitec.Caption := 'N';
      end;

      txtDataProcessamento_SIGCB.Caption := FormatDateTime('dd/mm/yyyy',Now);
      txtNossoNumeroc.Caption := ANossoNumero;
      txtUsoBanco_SIGCB.Caption := '';

      txtCarteira_SIGCB.Caption := ACarteira;
      txtCarteirac.Caption := ACarteira;

      txtEspecieMoeda_SIGCB.Caption := 'R$';
      txtQuantidadeMoeda_SIGCB.Caption := '';
      txtValorMoeda_SIGCB.Caption := '';

      txtvalor_titulo.Caption := FormatCurr('#,##0.00',ValorDocumento);
      txtValorDocumento_SIGCB.Caption := FormatCurr('#,##0.00',ValorDocumento);

      txtInstrucoes_SIGCB.Lines.Clear;
      txtInstrucoes_SIGCB.Lines.AddStrings(AInstrucoes);
      txtValorDescontoAbatimento_SIGCB.Caption := '';
      txtValorDescontoAbatimentoB_SIGCB.Caption := '';
      txtValorMoraMulta_SIGCB.Caption := '';
      txtValorMoraMultaB_SIGCB.Caption := '';
      txtValorCobrado_SIGCB.Caption := '';

      txtSacadoNomec.Caption := AnsiUpperCase(Sacado.Nome);
      txtSacadoNome_SIGCB.Caption := AnsiUpperCase(Sacado.Nome);
      
      case Sacado.TipoInscricao of
         tiPessoaFisica  : txtSacadoCPFCGC_SIGCB.Caption := 'CPF: ' + FormatarComMascara('!000\.000\.000\-00;0; ',Sacado.NumeroCPFCGC);
         tiPessoaJuridica: txtSacadoCPFCGC_SIGCB.Caption := 'CNPJ: ' + FormatarComMascara('!00\.000\.000\/0000\-00;0; ',Sacado.NumeroCPFCGC);
         tiOutro         : txtSacadoCPFCGC_SIGCB.Caption := Sacado.NumeroCPFCGC;
      end;

      case Sacado.TipoInscricao of
         tiPessoaFisica  : txtcpfcnpj.Caption := 'CPF: ' + FormatarComMascara('!000\.000\.000\-00;0; ',Sacado.NumeroCPFCGC);
         tiPessoaJuridica: txtcpfcnpj.Caption := 'CNPJ: ' + FormatarComMascara('!00\.000\.000\/0000\-00;0; ',Sacado.NumeroCPFCGC);
         tiOutro         : txtcpfcnpj.Caption := Sacado.NumeroCPFCGC;
      end;
      
      txtSacadoRuaNumeroComplementoc.Caption := AnsiUpperCase(Sacado.Endereco.Rua + ', ' + Sacado.Endereco.Numero + '   ' + Sacado.Endereco.Complemento);
      txtSacadoRuaNumeroComplemento_SIGCB.Caption := AnsiUpperCase(Sacado.Endereco.Rua + ', ' + Sacado.Endereco.Numero + '   ' + Sacado.Endereco.Complemento);

      txtSacadoBairroCidadeEstadoc.caption := AnsiUpperCase( Sacado.Endereco.Bairro + '    ' + Sacado.Endereco.Cidade + '    ' + Sacado.Endereco.Estado);
      txtSacadoCEPBairroCidadeEstadoc.caption :=FormatarComMascara('00000-000;0; ',Sacado.Endereco.CEP);
      txtSacadoCEPBairroCidadeEstado_SIGCB.Caption := AnsiUpperCase(FormatarComMascara('00000-000;0; ',Sacado.Endereco.CEP) + '    ' + Sacado.Endereco.Bairro + '    ' + Sacado.Endereco.Cidade + '    ' + Sacado.Endereco.Estado);

      txtLinhaDigitavel_SIGCB.Caption := CodigoBarra.LinhaDigitavel;
      imgCodigoBarras_SIGCB.Picture.Assign(CodigoBarra.Imagem.Picture);
   end;
   AInstrucoes.Free;
end;

procedure TgbTitulo.PrepararBoleto_sigcb(ABoleto: TBoleto_A4SIGCB);
var
   AAgenciaCodigoCedente,
   ANossoNumero,
   ACarteira,
...
//      imgCodigoBarras_SIGCB.Picture.Assign(CodigoBarra.Imagem.Picture);
      ...
      CodigoBarra.Imagem.Picture.SaveToFile(ExtractFilePath(ParamStr(0)) +'boletoconf.bmp');
      imgCodigoBarras_SIGCB.Picture.LoadFromFile(ExtractFilePath(ParamStr(0)) +'boletoconf.bmp');
      DeleteFile(ExtractFilePath(ParamStr(0)) +'boletoconf.bmp');
...
procedure TgbTitulo.Visualiazar_SIGCB;
begin
   FBoleto_SIGCB := TBoleto_A4SIGCB.Create(nil);
   TRY
      PrepararBoleto_sigcb(FBoleto_SIGCB);

   //   gerarPDF(FBoleto_SIGCB,'C:\nery','boleto');

      FBoleto_SIGCB.Preview;

//      Frmnovoassoc.EXQR.Report := FBoleto_SIGCB;
//      Frmnovoassoc.EXQR.ExportQR;
...
procedure TgbTitulo.Imprimir_SIGCB_MG;
var
   ABoleto : TBoleto_A4SIGCB_mg;
begin
   ABoleto := TBoleto_A4SIGCB_MG.Create(nil);
   TRY
      PrepararBoleto_sigcb_MG(ABoleto);
      ABoleto.ShowProgress := false;
      ABoleto.PrinterSettings.PrinterIndex := pRINTER.PrinterIndex;
      ABoleto.ReportTitle := NossoNumero;
      ABoleto.Print;
   ...
   END;
END;
procedure TgbTitulo.Visualiazar_SIGCB_MG;
begin
   FBoleto_SIGCB_MG := TBoleto_A4SIGCB_MG.Create(nil);
   TRY
      PrepararBoleto_sigcb_mg(FBoleto_SIGCB_MG);
      FBoleto_SIGCB_MG.Preview;
      FBoleto_SIGCB_MG.Free;
   EXCEPT
      FBoleto_SIGCB_MG.Free;
      Raise;
   END;
...
procedure TgbTitulo.Visualiazar_SIGCB_BB_MG;
begin
   FBoleto_SIGCB_BB_MG := TBoleto_A4SIGCB_BB_MG.Create(nil);
   TRY
      PrepararBoleto_sigcb_BB_mg(FBoleto_SIGCB_BB_MG);
      FBoleto_SIGCB_BB_MG.Preview;
      FBoleto_SIGCB_BB_MG.Free;
   EXCEPT
      FBoleto_SIGCB_BB_MG.Free;
      Raise;
   END;
...
procedure TgbTitulo.PrepararBoleto_sigcb_BB_mg(
  ABoleto: TBoleto_A4SIGCB_BB_MG);
var
   AAgenciaCodigoCedente,
   ANossoNumero,
   ACarteira,
...
begin
   ...
   GetFormatoBoleto := nil;
   if cedente.DigitoCodigoCedente <> '' then
      AAgenciaCodigoCedente := Cedente.ContaBancaria.CodigoAgencia +'-'+Cedente.ContaBancaria.DigitoAgencia+ '/' + FormatarComMascara('000000',Cedente.CodigoCedente) + '-'+ cedente.DigitoCodigoCedente
   else
      AAgenciaCodigoCedente := Cedente.ContaBancaria.CodigoAgencia +'-'+Cedente.ContaBancaria.DigitoAgencia+ '/' + FormatarComMascara('000000',Cedente.CodigoCedente);
...
procedure TgbTitulo.Imprimir_SIGCB_BB_MG;
var
   ABoleto : TBoleto_A4SIGCB_BB_mg;
begin
   ABoleto := TBoleto_A4SIGCB_BB_MG.Create(nil);
   TRY
      PrepararBoleto_sigcb_BB_MG(ABoleto);
      ABoleto.ShowProgress := false;
      ABoleto.PrinterSettings.PrinterIndex := pRINTER.PrinterIndex;
      ABoleto.ReportTitle := NossoNumero;
      ABoleto.Print;
...
procedure TgbTitulo.QRCompoBoletoAddReports(Sender: TObject);
begin

end;

procedure TgbTitulo.PrepararBoleto_sigcb_BB_mg_CTA2(
  ABoleto: TBoleto_A4SIGCB_BB_MG);
var
   AAgenciaCodigoCedente,
   ANossoNumero,
...
function TgbTitulo.GerarPDF_SIGCB(caminho, arquivo: string): integer;
begin
   FBoleto_SIGCB := TBoleto_A4SIGCB.Create(nil);
   TRY
      PrepararBoleto_sigcb(FBoleto_SIGCB);

      //result := gerarPDF(FBoleto_SIGCB,caminho,arquivo);
      result := GeraPDF(FBoleto_SIGCB,caminho,  arquivo);
      FBoleto_SIGCB.Free;
   EXCEPT
      FBoleto_SIGCB.Free;
      Raise;
...
procedure TgbTitulo.Imprimir_SIGCB_e_PDF(nomearquivo:string);
var
   ABoleto : TBoleto_A4SIGCB;
begin
   ABoleto := TBoleto_A4SIGCB.Create(nil);
...
procedure TgbTitulo.Visualiazar_SIGCB_e_PDF(nomearquivo: string);
var
   ABoleto : TBoleto_A4SIGCB;
begin
   ABoleto := TBoleto_A4SIGCB.Create(nil);
...
function TgbCobranca.LerRetorno_BB: boolean;
var
   ACodigoBanco: string;
   Retorno : TStringList;
   AClasseBanco: TPersistentClass;
...
begin
   ...
   TRY
      ...
      begin
         ABanco := AClasseBanco.Create;
         TRY
            @GetRetorno := ABanco.MethodAddress('LerRetorno_BB');
            if @GetRetorno <> nil then
               Result := GetRetorno(Self, Retorno)
            else
               Raise Exception.CreateFmt('O processamento de arquivo retorno do banco %s não está disponível',[ACodigoBanco]);
...
function TgbCobranca.LerRetorno_BB_AL: boolean;
var
   ACodigoBanco: string;
   Retorno : TStringList;
   AClasseBanco: TPersistentClass;
...
begin
   ...
   TRY
      ...
      begin
         ABanco := AClasseBanco.Create;
         TRY
            @GetRetorno := ABanco.MethodAddress('LerRetorno_BB_AL');
            if @GetRetorno <> nil then
               Result := GetRetorno(Self, Retorno)
            else
               Raise Exception.CreateFmt('O processamento de arquivo retorno do banco %s não está disponível',[ACodigoBanco]);
