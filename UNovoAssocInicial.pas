...
begin
    If dbtppessoa.text='Pessoa Jurídica' then
    begin
    ...
    If dbtppessoa.text='Pessoa Física' then
    begin
...
begin
  ...
  begin
     Aviso('CPF/CNPJ não pode ser vazio. ');
     edcpfcnpj.SetFocus;
     modalresult := mrNone;
  end
  else
  ...
  begin
     Aviso('Data do Registro Social não pode ser vazia. ');
  ...
  begin
     ...
     begin
         if not Confirma('Já existe um Registrado ativo com este CPF/CNPJ. '+#13+sdsVerCPFCNPJNOME.value+#13+'Deseja continuar mesmo assim? ') then
         begin
            edcpfcnpj.SetFocus;
            modalresult := mrNone;
         end;
  ...
  begin
     if Confirma('Já existe um filiado cadastrado com este número de registro.'+#13+'Deseja gerar o próximo número?') then
     begin
