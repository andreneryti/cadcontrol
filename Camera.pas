...
begin
  ...
  FAreaVideo := Self;
  FFramePreview := 0;
  FVideoDriverNombre := 'Não Existe driver';
...
begin
  inherited Paint;
  SetWindowPos(FFramePreview, HWND_TOP, -50, -50, width + 50, height + 50, SWP_SHOWWINDOW)
end;
...
begin
  ...
  begin
    ...
    raise Exception.Create('Sem conexão com a camera');
end;
...
begin
  ...
  begin
    ...
    raise Exception.Create('Não foi possivel conectar a camera. ');
end;
...
begin
  ...
  if retc = FALSO then
    showmessage(FVideoDriverNombre + ': Error em capFileSetCaptureFile');
...
begin
  ...
  if DialogDisplay then
    ...
    raise Exception.Create('Display inválido !');
...
procedure TCamera.SetCenter(const Value: Boolean);
begin
  FCenter := Value;
end;
